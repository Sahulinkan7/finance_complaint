...
class DataIngestion:
    def __init__(self,data_ingestion_config:DataIngestionConfig,n_retry:int=5):
        ...
        n_retry: number of retry to download files for failed encounters
        """
        try:
            logging.info(f"{'##'*30} Starting data Ingestion {'##'*30}")
    ...
    def get_required_interval(self):
        try:
            ...
            logging.info(f"Prepared Intervals : {intervals}")
    ...
    def download_files(self,n_day_interval_url:int=None):
        ...
        try:
            ...
            logging.info(f"Started downloading files")
            for index in range(1,len(required_interval)):
                from_date,to_date=required_interval[index-1],required_interval[index]
                logging.info(f"Generating data download url from {from_date} to {to_date} ")
    ...
    def convert_files_to_parquet(self)->str:
        """
        downloaded files will be converted to one single parquet file
        json_data_dir: downloaded json directory
        ...
        try:
            ...
            logging.info(f"Parquet file will be create at {file_path}")
            if not os.path.exists(json_data_dir):
                return file_path
            for file_name in os.listdir(json_data_dir):
                json_file_path=os.path.join(json_data_dir,file_name)
                logging.debug(f"converting {json_file_path} into parquet file at {file_path}")
    ...
    def retry_download_data(self,data,download_url: DownloadUrl):
        """
        downloads data for failed cases
        """
        try:
            if download_url.n_retry==0:
                self.failed_download_urls.append(download_url)
                logging.info(f"unable to download files {download_url.url}")
                return 
    ...
    def download_data(self,download_url:DownloadUrl):
        try:
            logging.info(f"Starting downloading from {download_url}")
            ...
            try:
                logging.info(f"Started writing downloaded data into json files")
                with open(download_url.file_path,"w") as file_obj:
                    financial_complaint_data=list(map(lambda x:x["_source"],
                                                    filter(lambda x:"_source" in x.keys(),
                                                    json.loads(data.content)))
                                                )
                    json.dump(financial_complaint_data,file_obj)
                logging.info(f"Downloaded data has been written into file {download_url.file_path}")
            except Exception as e:
                logging.info(f"Failed to download and retry download")
    ...
    def write_metadata(self,file_path:str)->None:
        try:
            logging.info(f"Writing meta data info into metadata file")
            ...
            logging.info(f"meta data written")
        except Exception as e:
            raise FinanceException(e, sys) from e
    ...
    def initiate_data_ingestion(self):
        try:
            ...
            if os.path.exists(self.data_ingestion_config.download_dir):
                logging.info(f"converting and combining downloaded json file into parquet file")
